...
};AF.Grid.extension={};}(jQuery));(function(C){if(!C.browser){var N=/MSIE(.*?);/.exec(navigator.appVersion);C.browser={msie:navigator&&navigator.appName.indexOf("Microsoft")>-1,version:N&&N[1]};}if(C.browser.msie&&parseInt(C.browser.version,10)<=7&&document.documentMode!==8){C("html").addClass("ie7below");
}C.afGrid=C.extend(C.afGrid,{appendRows:"afGrid-append-rows",destroy:"afGrid-destroy",renderingComplete:"afGrid-rendering-complete",adjustRowWidth:"afGrid-adjust-row-width",showLoading:"afGrid-showLoading",hideLoading:"afGrid-hideLoading",getElementId:function(T){var S=T.split("_");S.splice(0,1);return S.join("_");
}});C.afGrid.renderer={};C.afGrid.filter={};C.afGrid.plugin=C.afGrid.plugin||{};var O={};var B;var p="<div class='total-row-count'>Showing {loadedRows} of {totalRows}</div>",R="<div class='group-data'></div>",r="<div class='afGrid-heading'></div>",D="<div class='{cssClass} column-helper'></div>",P="<div class='afGrid-head'></div>",z="<div class='cell {columnId} {cssClass}' id='{id}'><span class='label'>{value}</span></div>",d="<div class='group level{level}'><div class='group-header group-header-row'><span class='open-close-indicator'>-</span></div></div>",l="<div class='row level{level}' id='{id}'></div>",E="<div class='cell {columnId} {cssClass} {spacerClass}'>{value}</div>",v="<div class='afGrid-rows'><div class='afGrid-rows-content'></div></div>",K="<div class='loading-message'>Loading...</div>";
var A="DataRow_";var k="Col_";C.fn.afGrid=function(T){T=C.extend({id:null,rows:[],columns:[],rowHover:true,onRowClick:null,onScrollToBottom:C.noop,columnWidthOverride:null,makeColumnDraggable:f,showTotalRows:true,groupDetails:null,groupDetailsInFirstColumnOnly:true,totalRowLabelTemplate:p,loadingMessage:K},T);
var S={};if(!(T.id&&/^[a-zA-Z0-9]*$/.test(T.id))){throw"You need to provide id for the afGrid and ensure that the id don't have any special characters in it.";}return this.each(function(){var ad=C(this);var U=T.groupDetails;h(T);L(T);var af={},Y,aa=e(T,af,U),ae=aa.$rowsMainContainer,ac=T.rows.length,W;
if(ad.hasClass("afGrid-initialized")){ae=ad.find(".afGrid-rows").empty().append(ae.children());aa.$rowsMainContainer=ae;Y=ad.find(".afGrid-head");}else{ad.trigger(C.afGrid.destroy);Y=H(ad,T).wrap(P).parent();ad.addClass("afGrid").empty().append(Y.add(ae));if(T.showTotalRows){ad.append(T.totalRowLabelTemplate.supplant({totalRows:"",loadedRows:""}));
}ad.append(T.loadingMessage);ae.bind("scroll.afGrid",function(){Q(Y,ae,T);ad.data("lastScrollPos",ae[0].scrollLeft);});ad.unbind(C.afGrid.hideLoading).bind(C.afGrid.hideLoading,function(){ad.find(".loading-message").hide();});ad.unbind(C.afGrid.showLoading).bind(C.afGrid.showLoading,function(){ad.find(".loading-message").show();
});}Z();if(ae&&ae.length){ae[0].scrollLeft=ad.data("lastScrollPos");}function ab(){G(S,T);q(ad,Y,ae);Y=null;aa=null;ae=null;T=null;ad=null;af=null;}ad.unbind(C.afGrid.destroy).bind(C.afGrid.destroy,ab);function X(ai,ah,ag){ac=w(ah,ac,ad,T,ag,aa,af,W,U);}ad.unbind(C.afGrid.appendRows).bind(C.afGrid.appendRows,X);
function Z(){var ag=0;Y.find(".afGrid-heading .cell:visible").each(function(){ag+=C(this).outerWidth(true);});ad.find(".afGrid-rows-content").css({minHeight:1,overflow:"hidden",width:ag});}ad.unbind(C.afGrid.adjustRowWidth).bind(C.afGrid.adjustRowWidth,Z);if(T.rowHover){ad.undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").delegate(".afGrid-rows .row","mouseenter",function(){C(this).addClass("row-hover");
}).delegate(".afGrid-rows .row","mouseleave",function(){C(this).removeClass("row-hover");});}if(T.onRowClick){ad.undelegate(".afGrid-rows .row","click").delegate(".afGrid-rows .row","click",function(){var ag=C.afGrid.getElementId(C(this).attr("id"));T.onRowClick(af[ag].orig||af[ag],C(this));});}t(ad,T,ac);
var V={getColumnElementById:J,getRowElementById:u,getColumnById:x,getCellContent:j};C.each(C.afGrid.plugin,function(ag,ah){if(ad.hasClass("afGrid-initialized")){S[ag]=O[T.id][ag];if(S[ag].update){S[ag].update(af);}}else{O[T.id]=O[T.id]||{};O[T.id][ag]=S[ag]=ah(ad,T,af);S[ag].load(V);}});ad.addClass("afGrid-initialized");
ad.trigger(C.afGrid.renderingComplete);});};function q(U,S,T){if(C.draggable){U.find(".afGrid-heading .cell").draggable("destroy");}S.undelegate().unbind().empty().remove();T.undelegate().unbind().remove();s(U);U.unbind(C.afGrid.hideLoading).unbind(C.afGrid.showLoading).unbind(C.afGrid.adjustRowWidth).unbind(C.afGrid.destroy).unbind(C.afGrid.appendRows).undelegate(".group .group-header","click").undelegate(".afGrid-rows .row","click").undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").empty();
U.data("afGridColumnDraggable",false);}function w(X,aa,ab,ae,V,Y,ad,W,U){aa+=X.length;t(ab,ae,aa);ae.columnWidthOverride=V;L(ae);var ac=Y.lastGroupInformation.$groupContainers,T=Y.lastGroupInformation.currentGroupValues,Z=Y.$rowsMainContainer,S=Z.find(".row:last").hasClass("even");Y.lastGroupInformation=i({tableId:ae.id,rows:X,columns:ae.columns,groups:ae.groupBy,$rowMainContainer:Z.find(".afGrid-rows-content"),$groupContainers:ac,currentGroupValues:T,isStartEven:S,cachedGridData:ad,rowWidth:W,groupDetails:U,areFetchedRows:true,groupDetailsInFirstColumnOnly:ae.groupDetailsInFirstColumnOnly});
if(ab.find(".afGrid-rows")[0].scrollHeight<=ab.find(".afGrid-rows").height()){ae.onScrollToBottom();}return aa;}function G(S,T){C.each(S,function(U,V){if(V.destroy){V.destroy();}});delete O[T.id];}function Q(S,U,T){var V=U[0];S.css({marginLeft:-1*V.scrollLeft});clearTimeout(B);B=setTimeout(function(){var W=(V.scrollHeight-U.scrollTop()),X=U.height();
...
}});}}function h(S){S.columnsHashMap={};C.each(S.columns,function(T,U){S.columnsHashMap[U.id]=T;});}function u(T,S){return C("#"+S.id+A+T);}function J(T,S){return C("#"+S.id+k+T);}function f(S){if(!S.data("afGridColumnDraggable")){if(C.fn.draggable){S.find(".afGrid-heading .cell").draggable({helper:function(T){return M(T,S.attr("class"));
},revert:false,cancel:".resize-handle",appendTo:"body",refreshPositions:true,cursorAt:{top:0,left:0}});}S.data("afGridColumnDraggable",true);}}function s(S){if(C.fn.draggable){S.find(".afGrid-heading .cell").draggable("destroy");}S.removeData("afGridColumnDraggable");}function M(T,S){return C(T.currentTarget).clone(false).wrap(r).parent().wrap(D.supplant({cssClass:S})).parent().css("width","auto");
...
if(C.afGrid.renderer[aa.type]&&C.afGrid.renderer[aa.type]["headerCell"]){Z.value=C.afGrid.renderer[aa.type].headerCell(V,aa,U,T);}ab=C(W.cell.supplant(Z));ab.css({width:aa.width,display:aa.isHidden?"none":""});S.append(ab);}});return S;}function e(T,V,U){var S=C(v),W=i({tableId:T.id,rows:T.rows,columns:T.columns,groups:T.groupBy,$rowMainContainer:S.find(".afGrid-rows-content"),$groupContainers:null,currentGroupValues:[],isStartEven:false,cachedGridData:V,rowWidth:null,groupDetails:U,areFetchedRows:false,groupDetailsInFirstColumnOnly:T.groupDetailsInFirstColumnOnly});
return{$rowsMainContainer:S,lastGroupInformation:W};}function x(T,S){return C.grep(S,function(U){return U["id"]===T;})[0];}function I(T,S){if(S){return C.grep(S,function(U){return U&&(U.refLabel===T);})[0];}return null;}function m(S,T,W){var V=I(S[0],T);for(var U=1;U<=W;U++){V=V&&I(S[U],V.groupDetails);
}return V;}function b(S,U,X,T){var V=m(S,U,X);var W=[];if(V){C.each(V,function(Y,aa){var Z=x(Y,T);if(Z){W[W.length]=Z.label+": "+j(aa,Z);}});return"["+W.join(", ")+"]";}return"";}function g(ad){var V=ad.n,X=ad.columns,ae=ad.$groupContainers,U=ad.groupDetails,T=ad.currentGroupValues,S,ab=C(),Z=ad.groupDetailsInFirstColumnOnly,Y;
...
if(aa===V){ac.addClass("first-cell-in-group");}ab=ab.add(ac);}ae[V].find(".group-header").append(ab).addClass(Z?"details-in-first-column":"");}function F(U,W,T,S){U[W]=C(d.supplant({level:W}));var V=C(R);if(W!==0){V.append(c(T[W-1],"","spacer"));}V.append(U[W]);S.append(V);S=U[W];return{$wrapper:V,$placeHolder:S};
}function a(U,W,T,S){U[W]=C(d.supplant({level:W}));var V=C(R);if(W===0){V.append(U[W]);S.append(V);}else{V.append(c(T[W-1],"","spacer"));V.append(U[W]);U[W-1].append(V);}return{$wrapper:V,$placeHolder:S};}function n(ae){var af=ae.$placeHolder,Y=ae.groups,S=ae.level,U=ae.currentGroupValues,aa=ae.columns,V=ae.groupDetails,X,Z,ac,ad,T,ab;
...
}if(aj===0&&!ab){ah.addClass("row-first");}if(Z){ah.css("width",Z);}ai.append(ah);});return{$groupContainers:ae,currentGroupValues:W};}function j(S,U,T,W){var V=(U.type||U.renderer);if(V){S=C.afGrid.renderer[V].cell(S,U,T,W);}return S;}function y(V,ab,S,Y){var U=ab.id;var X=U?ab.data:ab,Z=C(l.supplant({level:S,id:V+A+U})),aa=Y[S-1];
...
if(j&&j.indexOf(r)>-1){return false;}j.push(r);setTimeout(function(){n.removeClass("afGrid-initialized");o.onGroupChange(j);},10);return true;}function h(s){var t=d(s.currentTarget),q=!t.data("state");t.data("state",q);t.find(".open-close-indicator").html(q?"+":"-");t.removeClass("open close").addClass(q?"close":"open");
var r=t.parents(".group").eq(0);if(q){r.addClass("group-closed");setTimeout(function(){r.addClass("group-closed-complete");},300);}else{r.removeClass("group-closed-complete");setTimeout(function(){r.removeClass("group-closed");},10);}setTimeout(function(){n.find(".group-data").css({zoom:1}).css({zoom:0});
if(n.find(".afGrid-rows")[0].scrollHeight<=n.find(".afGrid-rows").height()){o.onScrollToBottom();}},450);}function i(s,t){var u=d(s.target);u.removeClass("reorder");var r=d.afGrid.getElementId(t.draggable.attr("id")),v=d.afGrid.getElementId(u.attr("id")),q=[];d.each(o.groupBy,function(w,x){if(x!==r){q.push(x);
}if(x===v){q.push(r);}});n.removeClass("afGrid-initialized");o.onGroupReorder(q);}function m(q){if(!o.isGridGroupable){return;}d.each(o.columns,function(r,s){if(!(d.afGrid.renderer[s.type]&&d.afGrid.renderer[s.type].headerCell)){if(s.isGroupable!==false){q.getColumnElementById(s.id,o).append("<span class='groupable-indicator'></span>").addClass("groupable-column");
...
}}});function a(f){d(f.target).addClass("reorder");}function c(f){d(f.target).removeClass("reorder");}function e(f){return d(f.currentTarget).clone(false).addClass("group-helper");}}(jQuery));(function(a){a.afGrid.updateRow="afGrid-update-row";a.afGrid=a.extend(true,a.afGrid,{plugin:{realTimeData:function(e,b,d){b=a.extend({isLive:true},b);
...
}}d[j.id].orig[l]=o;}n.html(h.getCellContent(o,m));window.setTimeout(function(){n.removeClass("updated-cell");n=null;},2000);}});}e.unbind(a.afGrid.updateRow).bind(a.afGrid.updateRow,g);}function c(){e.unbind(a.afGrid.updateRow);b=null;}return{load:f,destroy:c};}}});}(jQuery));(function(a){a.afGrid=a.extend(true,a.afGrid,{plugin:{sortable:function(f,c){c=a.extend({isGridSortable:true,sortBy:null,onSort:a.noop},c);
...
}function d(){b("."+c.id+"-afGrid-switch-theme").undelegate(".theme","click");c=null;}return{load:f,destroy:d};}}});}(jQuery));(function(b){b.afGrid.renderer.DATE={cell:function(c){return"<span class='date'>{date}</span>".supplant({date:c?c:"&nbsp;"});},comparator:function(d,c){return a(d)-a(c);}};function a(d){var c=/(\d{1,2})\/(\d{2})\/(\d{4})/.exec(d);
...
(function(a){a.afGrid.renderer.NUMERIC={cell:function(b){return b?b.toFixed(2):"";},comparator:function(c,b){c=Number(String(c).replace(/,/g,""));b=Number(String(b).replace(/,/g,""));return c-b;}};}(jQuery));(function(b){var a=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
b.afGrid.renderer.SYSTEM_DATE={cell:function(d){var c=d.split("-");var e=d!=="-"?[c[2],"-",a[(+c[1])-1],"-",c[0]].join(""):"-";return"<span class='date'>{date}</span>".supplant({date:e});}};}(jQuery));