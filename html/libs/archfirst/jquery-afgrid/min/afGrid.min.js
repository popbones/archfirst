...
}return this.each(function(){var Q=f(this);v(H);j(H);var S={},L,N=i(H,S),R=N.$rowsMainContainer,P=H.rows.length,J;if(Q.hasClass("afGrid-initialized")){R=Q.find(".afGrid-rows").empty().append(R.children());N.$rowsMainContainer=R;L=Q.find(".afGrid-head");}else{Q.trigger(f.afGrid.destroy);L=B(Q,H).wrap(w).parent();
Q.addClass("afGrid").empty().append(L.add(R));if(H.showTotalRows){Q.append(H.totalRowLabelTemplate.supplant({totalRows:"",loadedRows:""}));}Q.append(H.loadingMessage);R.bind("scroll.afGrid",function(){C(L,R,H);Q.data("lastScrollPos",R[0].scrollLeft);});Q.unbind(f.afGrid.hideLoading).bind(f.afGrid.hideLoading,function(){Q.find(".loading-message").hide();
});Q.unbind(f.afGrid.showLoading).bind(f.afGrid.showLoading,function(){Q.find(".loading-message").show();});}M();if(R&&R.length){R[0].scrollLeft=Q.data("lastScrollPos");}function O(){q(F,H);if(f.draggable){Q.find(".afGrid-heading .cell").draggable("destroy");}L.undelegate().unbind().empty().remove();
L=null;N=null;R.undelegate().unbind().remove();R=null;p(Q);Q.unbind(f.afGrid.hideLoading).unbind(f.afGrid.showLoading).unbind(f.afGrid.adjustRowWidth).unbind(f.afGrid.destroy).unbind(f.afGrid.appendRows).undelegate(".group .group-header","click").undelegate(".afGrid-rows .row","click").undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").empty();
