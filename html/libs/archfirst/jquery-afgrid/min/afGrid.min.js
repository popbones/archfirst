...
}l.load(k,u);});}function D(){b();if(l){l.destroy();l=null;}}function h(){f.trigger(a.afGrid.destroy);f.removeClass("afGrid-initialized");if(l){l.refresh(k,u);}}function b(){if(e.statePersist){e.statePersist.save("afGridState_"+e.id,null);}k={};k.pageSize=e.pageSize;f.trigger(a.afGrid.destroy);f.removeClass("afGrid-initialized");
}function c(I){e.groupBy=[];e.state={};e.columnWidthOverride=null;e.sortBy=[];e.filterBy=[];b();k=I||null;if(l){l.refresh(k,u);}}function s(){return e.dataSource;}function v(){return l.getCurrentMetaData();}function t(J){var I=a.extend(e,J);f=a(e.afGridSelector);f.afGrid(I);}function d(I){f.trigger(a.afGrid.appendRows,[I.rows,k.columnWidthOverride]);
}function q(){return B;}function x(){return a.extend({},e);}return a.extend({},AF.Grid.extension,{load:o,destroy:D,refresh:h,resetAndRefresh:c,getDefaultOptions:q,getCurrentOptions:x,getCurrentMetaData:v,getSource:s});};AF.Grid.extension={};}(jQuery));(function(f){if(f.browser.msie&&parseInt(f.browser.version.slice(0,1),10)<=7&&document.documentMode!==8){f("html").addClass("ie7below");
}f.afGrid=f.extend(f.afGrid,{appendRows:"afGrid-append-rows",destroy:"afGrid-destroy",renderingComplete:"afGrid-rendering-complete",adjustRowWidth:"afGrid-adjust-row-width",showLoading:"afGrid-showLoading",hideLoading:"afGrid-hideLoading",getElementId:function(G){var F=G.split("_");F.splice(0,1);return F.join("_");
}});f.afGrid.renderer={};f.afGrid.filter={};f.afGrid.plugin=f.afGrid.plugin||{};var x={};var l;var s="<div class='total-row-count'>Showing {loadedRows} of {totalRows}</div>",t="<div class='group-data'></div>",c="<div class='afGrid-heading'></div>",g="<div class='{cssClass} column-helper'></div>",w="<div class='afGrid-head'></div>",k="<div class='cell {columnId} {cssClass}' id='{id}'><span class='label'>{value}</span></div>",r="<div class='group level{level}'><div class='group-header'><span class='open-close-indicator'>-</span>{value}</div></div>",m="<div class='row level{level}' id='{id}'></div>",o="<div class='cell {columnId} {cssClass} {spacerClass}'>{value}</div>",z="<div class='afGrid-rows'><div class='afGrid-rows-content'></div></div>",A="<div class='loading-message'>Loading...</div>";
f.fn.afGrid=function(H){H=f.extend({id:null,rows:[],columns:[],rowHover:true,onRowClick:null,onScrollToBottom:f.noop,columnWidthOverride:null,makeColumnDraggable:E,showTotalRows:true,totalRowLabelTemplate:s,loadingMessage:A},H);var F={},G=[];if(!(H.id&&/^[a-zA-Z0-9]*$/.test(H.id))){throw"You need to provide id for the afGrid and ensure that the id don't have any special characters in it.";
}return this.each(function(){var Q=f(this);if(!Q.hasClass("afGrid-initialized")){Q.trigger(f.afGrid.destroy);Q=f(this);}v(H);j(H);var S={},L,N=i(H,S),R=N.$rowsMainContainer,P=H.rows.length,J;if(Q.hasClass("afGrid-initialized")){R=Q.find(".afGrid-rows").empty().append(R.children());N.$rowsMainContainer=R;
L=Q.find(".afGrid-head");}else{L=B(Q,H).wrap(w).parent();Q.addClass("afGrid").empty().append(L.add(R));if(H.showTotalRows){Q.append(H.totalRowLabelTemplate.supplant({totalRows:"",loadedRows:""}));}Q.append(H.loadingMessage);R.bind("scroll.afGrid",function(){C(L,R,H);Q.data("lastScrollPos",R[0].scrollLeft);
});Q.unbind(f.afGrid.hideLoading).bind(f.afGrid.hideLoading,function(){Q.find(".loading-message").hide();});Q.unbind(f.afGrid.showLoading).bind(f.afGrid.showLoading,function(){Q.find(".loading-message").show();});}M();if(R&&R.length){R[0].scrollLeft=Q.data("lastScrollPos");}function O(){q(F,H);if(f.draggable){Q.find(".afGrid-heading .cell").draggable("destroy");
}L.undelegate().unbind().empty().remove();L=null;N=null;R.undelegate().unbind().remove();R=null;p(Q);Q.unbind(f.afGrid.hideLoading).unbind(f.afGrid.showLoading).unbind(f.afGrid.adjustRowWidth).unbind(f.afGrid.destroy).unbind(f.afGrid.appendRows).undelegate(".group .group-header","click").undelegate(".afGrid-rows .row","click").undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").empty();
Q.data("afGridColumnDraggable",false);H=null;Q=null;S=null;}Q.unbind(f.afGrid.destroy).bind(f.afGrid.destroy,O);function K(X,W,T){G=f.merge(G,W);P+=W.length;b(Q,H,P);H.columnWidthOverride=T;j(H);var V=N.lastGroupInformation.$groupContainers,Z=N.lastGroupInformation.currentGroupValues,U=N.$rowsMainContainer,Y=U.find(".row:last").hasClass("even");
N.lastGroupInformation=h(H.id,W,H.columns,H.groupBy,U.find(".afGrid-rows-content"),V,Z,Y,S,J);if(Q.find(".afGrid-rows")[0].scrollHeight<=Q.find(".afGrid-rows").height()){H.onScrollToBottom();}}Q.unbind(f.afGrid.appendRows).bind(f.afGrid.appendRows,K);function M(){var T=0;L.find(".afGrid-heading .cell:visible").each(function(){T+=f(this).outerWidth(true);
});Q.find(".afGrid-rows-content").css({minHeight:1,overflow:"hidden",width:T});}Q.unbind(f.afGrid.adjustRowWidth).bind(f.afGrid.adjustRowWidth,M);if(H.rowHover){Q.undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").delegate(".afGrid-rows .row","mouseenter",function(){f(this).addClass("row-hover");
}).delegate(".afGrid-rows .row","mouseleave",function(){f(this).removeClass("row-hover");});}if(H.onRowClick){Q.undelegate(".afGrid-rows .row","click").delegate(".afGrid-rows .row","click",function(){var T=f.afGrid.getElementId(f(this).attr("id"));H.onRowClick(S[T].orig||S[T],f(this));});}b(Q,H,P);var I={getColumnElementById:d};
f.each(f.afGrid.plugin,function(T,U){if(Q.hasClass("afGrid-initialized")){F[T]=x[H.id][T];if(F[T].update){F[T].update(S);}}else{x[H.id]=x[H.id]||{};x[H.id][T]=F[T]=U(Q,H,S);F[T].load(I);}});Q.addClass("afGrid-initialized");Q.trigger(f.afGrid.renderingComplete);});};function q(F,G){f.each(F,function(H,I){if(I.destroy){I.destroy();
