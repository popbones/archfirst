(function(a){window.AF=window.AF||{};AF.Grid=function(e){var A={id:null,dataSource:null,statePersist:a.statePersistToCookie,isGridGroupable:true,isGridSortable:true,groupsPlaceHolder:"."+e.id+"-afGrid-group-by",columnWidthOverride:null,pageSize:null,afGridSelector:"#"+e.id,onRowClick:a.noop,onSort:F,onGroupChange:r,onGroupReorder:x,onFilter:y,onColumnReorder:z,onColumnResize:m,onScrollToBottom:i,onStateChange:a.noop,onReset:c,onRefresh:h,groupBy:[],clientCache:false};
e=a.extend(true,{},A,e);var f=a();var l=new AF.Grid.DataStore(e.dataSource,e.clientCache),w=0,p=0,E=0,g=null,k;function G(H){g=H.columns;p=H.totalRows;w=H.rows.length;E=H.pageSize||e.pageSize;H.columnWidthOverride=H.columnWidthOverride||k.columnWidthOverride;if(H.groupBy&&H.groupBy.length){H.groupBy=a.map(H.groupBy,function(I){return I.id;
});}if(H.sortBy&&H.sortBy.length){H.sortBy={column:H.sortBy[0].id,direction:H.sortBy[0].direction};}t(H);k.columnOrder=a.map(H.columns,function(I){return I.id;});k.hiddenColumns=H.hiddenColumns;B();f.trigger(a.afGrid.hideLoading);}function B(){if(e.statePersist){e.statePersist.save("afGridState_"+e.id,JSON.stringify(k));
}e.onStateChange(k);}function D(H){if(e.statePersist){e.statePersist.load("afGridState_"+e.id,function(I){H(JSON.parse(I));});}else{H({});}}function i(){if(w>=p){return;}var H=a.extend({},k,{pageOffset:w+1,pageSize:E});f.trigger(a.afGrid.showLoading);l.fetchRows(H,n);}function n(H){k=k||{};w+=H.rows.length;
d(H);f.trigger(a.afGrid.hideLoading);}function u(H){k=k||{};k.pageSize=e.pageSize=H.pageSize||k.pageSize;G(H);}function y(H){k.filterBy=H;f.trigger(a.afGrid.showLoading);l.filter(k,G);}function r(H){var I;if(k.columnOrder){I=[];a.each(H,function(J,K){I.push(K);});a.each(k.columnOrder,function(J,K){if(I.indexOf(K)<0){I.push(K);
}});k.columnOrder=I;}k.groupBy=H.length?a.map(H,function(J){return{id:J,direction:"desc"};}):[];f.trigger(a.afGrid.showLoading);l.groupBy(k,G);}function F(H,I){k.sortBy=[{id:H,direction:I}];f.trigger(a.afGrid.showLoading);l.sortBy(k,G);}function z(J){var I,L,K,H;if(k.groupBy&&k.groupBy.length){I=k.groupBy.length;
L=[];for(K=0;K<I;K+=1){H=j(J[K]);if(H.column.isGroupable){L.push(J[K]);}else{break;}}k.groupBy=a.map(L,function(M){return{id:M,direction:"desc"};});}k.columnOrder=J;f.trigger(a.afGrid.showLoading);l.reorderColumn(k,G);}function j(I){var J=-1,H=a.grep(g,function(L,K){if(L.id===I){J=K;return true;}return false;
})[0];return{column:H,index:J};}function x(H){r(H);}function m(J,H,I){k.columnWidthOverride=k.columnWidthOverride||{};k.columnWidthOverride[J]=I;B();}function o(H){D(function(I){k=H||I||{};if(e.pageSize){k.pageSize=e.pageSize;}l.load(k,u);});}function C(){b();if(l){l.destroy();l=null;}}function h(){f.trigger(a.afGrid.destroy);
f.removeClass("afGrid-initialized");if(l){l.refresh(k,u);}}function b(){if(e.statePersist){e.statePersist.save("afGridState_"+e.id,null);}k={};k.pageSize=e.pageSize;f.trigger(a.afGrid.destroy);f.removeClass("afGrid-initialized");}function c(H){e.groupBy=[];e.state={};e.columnWidthOverride=null;e.sortBy=[];
e.filterBy=[];b();k=H||null;if(l){l.refresh(k,u);}}function s(){return e.dataSource;}function v(){return l.getCurrentMetaData();}function t(I){var H=a.extend(e,I);f=a(e.afGridSelector);f.afGrid(H);}function d(H){f.trigger(a.afGrid.appendRows,[H.rows,k.columnWidthOverride]);}function q(){return A;}return a.extend({},AF.Grid.extension,{load:o,destroy:C,refresh:h,resetAndRefresh:c,getDefaultOptions:q,getCurrentMetaData:v,getSource:s});
};AF.Grid.extension={};}(jQuery));(function(f){if(f.browser.msie&&parseInt(f.browser.version.slice(0,1),10)<=7&&document.documentMode!==8){f("html").addClass("ie7below");}f.afGrid=f.extend(f.afGrid,{appendRows:"afGrid-append-rows",destroy:"afGrid-destroy",renderingComplete:"afGrid-rendering-complete",adjustRowWidth:"afGrid-adjust-row-width",showLoading:"afGrid-showLoading",hideLoading:"afGrid-hideLoading",getElementId:function(G){var F=G.split("_");
F.splice(0,1);return F.join("_");}});f.afGrid.renderer={};f.afGrid.filter={};f.afGrid.plugin=f.afGrid.plugin||{};var x={};var l;var s="<div class='total-row-count'>Showing {loadedRows} of {totalRows}</div>",t="<div class='group-data'></div>",c="<div class='afGrid-heading'></div>",g="<div class='{cssClass} column-helper'></div>",w="<div class='afGrid-head'></div>",k="<div class='cell {columnId} {cssClass}' id='{id}'><span class='label'>{value}</span></div>",r="<div class='group level{level}'><div class='group-header'><span class='open-close-indicator'>-</span>{value}</div></div>",m="<div class='row level{level}' id='{id}'></div>",o="<div class='cell {columnId} {cssClass} {spacerClass}'>{value}</div>",z="<div class='afGrid-rows'><div class='afGrid-rows-content'></div></div>",A="<div class='loading-message'>Loading...</div>";
f.fn.afGrid=function(H){H=f.extend({id:null,rows:[],columns:[],rowHover:true,onRowClick:null,onScrollToBottom:f.noop,columnWidthOverride:null,makeColumnDraggable:E,showTotalRows:true,totalRowLabelTemplate:s,loadingMessage:A},H);var F={},G=[];v(H);j(H);if(!(H.id&&/^[a-zA-Z0-9]*$/.test(H.id))){throw"You need to provide id for the afGrid and ensure that the id don't have any special characters in it.";
}return this.each(function(){var R=f(this);if(!R.hasClass("afGrid-initialized")){R.trigger(f.afGrid.destroy);R=f(this);}var K={},M,O=i(H,K),S=O.$rowsMainContainer,Q=H.rows.length,J;if(R.hasClass("afGrid-initialized")){S=R.find(".afGrid-rows").empty().append(S.children());O.$rowsMainContainer=S;M=R.find(".afGrid-head");
}else{M=B(R,H).wrap(w).parent();R.addClass("afGrid").empty().append(M.add(S));if(H.showTotalRows){R.append(H.totalRowLabelTemplate.supplant({totalRows:"",loadedRows:""}));}R.append(H.loadingMessage);S.bind("scroll.afGrid",function(){C(M,S,H);R.data("lastScrollPos",S[0].scrollLeft);});R.unbind(f.afGrid.hideLoading).bind(f.afGrid.hideLoading,function(){R.find(".loading-message").hide();
});R.unbind(f.afGrid.showLoading).bind(f.afGrid.showLoading,function(){R.find(".loading-message").show();});}N();if(S&&S.length){S[0].scrollLeft=R.data("lastScrollPos");}function P(){q(F,H);if(f.draggable){R.find(".afGrid-heading .cell").draggable("destroy");}M.undelegate().unbind().empty().remove();
M=null;O=null;S.undelegate().unbind().remove();S=null;p(R);R.unbind(f.afGrid.hideLoading).unbind(f.afGrid.showLoading).unbind(f.afGrid.adjustRowWidth).unbind(f.afGrid.destroy).unbind(f.afGrid.appendRows).undelegate(".group .group-header","click").undelegate(".afGrid-rows .row","click").undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").empty();
R.data("afGridColumnDraggable",false);H=null;R=null;K=null;}R.unbind(f.afGrid.destroy).bind(f.afGrid.destroy,P);function L(X,W,T){G=f.merge(G,W);Q+=W.length;b(R,H,Q);H.columnWidthOverride=T;j(H);var V=O.lastGroupInformation.$groupContainers,Z=O.lastGroupInformation.currentGroupValues,U=O.$rowsMainContainer,Y=U.find(".row:last").hasClass("even");
O.lastGroupInformation=h(H.id,W,H.columns,H.groupBy,U.find(".afGrid-rows-content"),V,Z,Y,K,J);if(R.find(".afGrid-rows")[0].scrollHeight<=R.find(".afGrid-rows").height()){H.onScrollToBottom();}}R.unbind(f.afGrid.appendRows).bind(f.afGrid.appendRows,L);function N(){var T=(function(){var U=0;M.find(".afGrid-heading .cell:visible").each(function(){U+=f(this).outerWidth(true);
});return U;}());R.find(".afGrid-rows-content").width(T).css({minHeight:1,overflow:"hidden"});J=0;R.find(".afGrid-heading .cell:visible").each(function(){J+=f(this).outerWidth();});R.find(".afGrid-rows .row").width(J);}R.unbind(f.afGrid.adjustRowWidth).bind(f.afGrid.adjustRowWidth,N);if(H.rowHover){R.undelegate(".afGrid-rows .row","mouseenter").undelegate(".afGrid-rows .row","mouseleave").delegate(".afGrid-rows .row","mouseenter",function(){f(this).addClass("row-hover");
}).delegate(".afGrid-rows .row","mouseleave",function(){f(this).removeClass("row-hover");});}if(H.onRowClick){R.undelegate(".afGrid-rows .row","click").delegate(".afGrid-rows .row","click",function(){var T=f.afGrid.getElementId(f(this).attr("id"));H.onRowClick(K[T].orig||K[T],f(this));});}b(R,H,Q);var I={getColumnElementById:d};
f.each(f.afGrid.plugin,function(T,U){if(R.hasClass("afGrid-initialized")){F[T]=x[H.id][T];if(F[T].update){F[T].update(K);}}else{x[H.id]=x[H.id]||{};x[H.id][T]=F[T]=U(R,H,K);F[T].load(I);}});R.addClass("afGrid-initialized");R.trigger(f.afGrid.renderingComplete);});};function q(F,G){f.each(F,function(H,I){if(I.destroy){I.destroy();
}});delete x[G.id];}function C(F,H,G){var I=H[0];F.css({marginLeft:-1*I.scrollLeft});clearTimeout(l);l=setTimeout(function(){var J=(I.scrollHeight-H.scrollTop()),K=H.height();if((J<=(K-17))||(J<=(K))){G.onScrollToBottom();}},100);}function b(G,F,H){if(F.showTotalRows){G.find(".total-row-count").replaceWith(F.totalRowLabelTemplate.supplant({totalRows:F.totalRows,loadedRows:H}));
}}function j(F){if(F.columnWidthOverride){f.each(F.columnWidthOverride,function(H,G){F.columns[F.columnsHashMap[H]].width=G;});}}function v(F){F.columnsHashMap={};f.each(F.columns,function(G,H){F.columnsHashMap[H.id]=G;});}function d(G,F){return f("#"+F.id+"Col_"+G);}function E(F){if(!F.data("afGridColumnDraggable")){if(f.fn.draggable){F.find(".afGrid-heading .cell").draggable({helper:function(G){return D(G,F.attr("class"));
},revert:false,cancel:".resize-handle",appendTo:"body",refreshPositions:true,cursorAt:{top:0,left:0}});}F.data("afGridColumnDraggable",true);}}function p(F){if(f.fn.draggable){F.find(".afGrid-heading .cell").draggable("destroy");}F.removeData("afGridColumnDraggable");}function D(G,F){return f(G.currentTarget).clone(false).wrap(c).parent().wrap(g.supplant({cssClass:F})).parent().css("width","auto");
}function B(G,F){return u(G,F.columns,{container:c,cell:k,cellContent:function(H){return{value:H.label,id:F.id+"Col_"+H.id,cssClass:H.type||H.renderer||"",columnId:H.id};}},F.id);}function u(I,H,J,G){var F=f(J.container),K=H.length;f.each(H,function(L,N){if(N.render!==false){var M=J.cellContent(N),O;
M.cssClass=M.cssClass+(L===K-1?" last":"")+(L===0?" first":"");if(f.afGrid.renderer[N.type]&&f.afGrid.renderer[N.type].headerCell){M.value=f.afGrid.renderer[N.type].headerCell(I,N,H,G);}O=f(J.cell.supplant(M));O.css({width:N.width,display:N.isHidden?"none":""});F.append(O);}});return F;}function i(G,I){var F=f(z),H=h(G.id,G.rows,G.columns,G.groupBy,F.find(".afGrid-rows-content"),null,[],false,I);
return{$rowsMainContainer:F,lastGroupInformation:H};}function y(J,F,H){var G=F[J];var I=(H[J].type||H[J].renderer);if(I){G=f.afGrid.renderer[I].cell(G,H[J]);}return f(r.supplant({value:G,level:J}));}function e(N,H,F,K,M,I){var G,J,L;if(M===null){M=[];for(G=0,J=H.length;G<J;G+=1){M[G]=y(G,K,I);L=f(t);
if(G!==0){L.append(n(I[G-1],"","spacer"));}L.append(M[G]);N.append(L);N=M[G];}}else{for(G=F,J=H.length;G<J;G+=1){M[G]=y(G,K,I);L=f(t);if(G===0){L.append(M[G]);N.append(L);}else{L.append(n(I[G-1],"","spacer"));L.append(M[G]);M[G-1].append(L);}}}return M;}function h(K,Q,M,J,O,P,I,F,N,L){var G=J&&J.length;
var H=new Date().getTime()+Math.floor(Math.random()*100);f.each(Q,function(T,W){W.id=W.id||K+"Row"+(H+T);var V=W.id,U=V?W.data:W,S=O,R;if(V){(N[V]=W);}if(G){if(P===null){f.each(J,function(X){I[X]=U[X];});P=e(O,J,null,I,P,M);}else{f.each(J,function(Y){var X;if(U[Y]!==I[Y]){for(X=Y;X<G;X+=1){I[X]=U[X];
}P=e(O,J,Y,I,P,M);}});}S=P[G-1];}R=a(K,V,W,G,M);if((T+(F?1:0))%2===0){R.addClass("even");}if(T===0&&L===undefined){R.addClass("row-first");}if(L){R.css("width",L);}S.append(R);});return{$groupContainers:P,currentGroupValues:I};}function a(J,I,Q,G,M){I=Q.id;var L=I?Q.data:Q,N=f(m.supplant({level:G,id:J+"DataRow_"+I})),O=M[G-1];
var H,K,F;if(O){N.append(n(O,""));}for(H=G,K=L.length;H<K;H+=1){F=L[H];var P=(M[H].type||M[H].renderer);if(P){F=f.afGrid.renderer[P].cell(F,M[H],H,Q);}N.append(n(M[H],F,null,H===(K-1)));}return N;}function n(F,J,I,H){var G=f(o.supplant({value:J,columnId:F.id,cssClass:(F.type||F.renderer||"")+(H?" last":""),spacerClass:I||""}));
G.css({"width":F.width,"display":F.isHidden?"none":""});G.css("width",F.width);return G;}}(jQuery));if(!Array.indexOf){Array.prototype.indexOf=function(c,d){var b,a;for(b=(d||0),a=this.length;b<a;b+=1){if(this[b]===c){return b;}}return -1;};}if(!String.hasOwnProperty("supplant")){String.prototype.supplant=function(a,b){return this.replace(/\{([^{}]*)\}/g,function(c,e){var f=b?e.replace(b+".",""):e,d=a[f];
...
if(d.fn.droppable){k.droppable({drop:p,accept:"#"+o.id+" .groupable-column",activeClass:"ui-state-highlight",tolerance:"pointer"});k.find(".cell").droppable({accept:".groups .cell",drop:i,over:a,out:c,tolerance:"pointer"});}if(d.fn.draggable){k.find(".cell").draggable({drop:p,helper:e,accept:"#"+o.id+" .groupable-column",containment:k});
...
}}return c;},destroy:function(d,c){c.find(".afGrid-rows").unbind("scroll.datefilter-"+d.attr("id"));d.daterangepicker("destroy");}};function a(d){var c=/(\d{1,2})\/(\d{1,2})\/(\d{4})/.exec(d);return new Date(c[3],c[2]-1,c[1]);}}(jQuery));(function(a){a.afGrid=a.afGrid||{};a.afGrid.filter=a.afGrid.filter||{};
a.afGrid.filter.DATERANGE={render:function(c){var b="<input id='{id}' type='text' class='DATERANGE-filter' value=''/>";return b.supplant({id:c});},init:function(c,b){c.daterangepicker({onClose:function(){c.attr("title",c.val());b();},dateFormat:"dd-M-yy"});},filterBy:function(c,b){c.val(b.value);c.attr("title",c.val());
},getValue:function(b){return b.val();},destroy:function(b){b.daterangepicker("destroy");}};}(jQuery));(function(b){var a={};var c=600;b.afGrid.filter.FREE_TEXT={render:function(e){var d="<input id='{id}' type='text' class='{filterIdentifier}'/>";return d.supplant({id:e});},init:function(g,f,d){var e=null;
...
},getValue:function(e){var d=e.val();return d&&{value:d};},destroy:function(d){d.unbind("change.filter").unbind("keyup.filter");}};}(jQuery));(function(b){b.afGrid.filter.MULTI_SELECT={render:function(f,e){var c,d;c="<select id='{id}' multiple='true' class='{filterIdentifier}'>{options}</select>";d=[];
b.each(e,function(g,h){d[d.length]="<option value='{value}'>{value}</option>".supplant({value:h});});return c.supplant({id:f,options:d.join("")});},init:function(f,e,c,d){if(b.fn.multiselect){f.multiselect({overrideWidth:"100%",overrideMenuWidth:"200px",close:g,click:a,checkAll:a,uncheckAll:a,noneSelectedText:"&nbsp;",selectedText:"Filtered",selectedList:1});
...
},init:function(c,b){c.datepicker({onClose:b,dateFormat:"dd-M-yy"});},filterBy:function(c,b){c.datepicker("setDate",b.value);},getValue:function(c){var b=c.val();return b&&{value:b};},destroy:function(b){b.datepicker("destroy");}};}(jQuery));(function(a){a.afGrid=a.extend(true,a.afGrid,{plugin:{LABEL_BUTTON:function(d,b,f){b=a.extend({labelButtonIdentifier:".button"},b);
function e(){d.delegate(b.labelButtonIdentifier,"click",function(){var g=a(this).closest(".row").attr("id").split("_")[1];alert("Cell data: "+a(this).next().val()+", Row data: "+JSON.stringify(f[g]));return false;});}function c(){d.undelegate(b.labelButtonIdentifier,"click");b=null;}return{load:e,destroy:c};
}}});}(jQuery));(function(b){var a={};b.afGrid=b.extend(true,b.afGrid,{plugin:{THEME_SWITCHER:function(e,c){function f(){var g=b("."+c.id+"-afGrid-switch-theme");g.delegate(".theme","click",function(){var k=b(this).attr("href").replace("#",""),j=c.id;var h=b("#"+j);var i=b("."+j+"-afGrid-group-by");if(a[j]){h.removeClass("afGrid-"+a[j]);
i.removeClass("afGrid-group-by-"+a[j]);}a[j]=k;h.addClass("afGrid-"+a[j]);i.addClass("afGrid-group-by-"+a[j]);return false;});}function d(){b("."+c.id+"-afGrid-switch-theme").undelegate(".theme","click");c=null;}return{load:f,destroy:d};}}});}(jQuery));(function(b){b.afGrid.renderer.DATE={cell:function(c){return"<span class='date'>{date}</span>".supplant({date:c});
},comparator:function(d,c){return a(d)-a(c);}};function a(d){var c=/(\d{1,2})\/(\d{2})\/(\d{4})/.exec(d);return c?new Date(c[3],parseInt(c[2],10)-1,c[1]).getTime():d;}}(jQuery));(function(a){a.afGrid.renderer.LABEL_BUTTON={cell:function(b){return"<input type='button' class='button' value='{value}'><input type='hidden' value='{actualValue}' class='dataValue'>".supplant({value:"Show",actualValue:b});
}};}(jQuery));(function(a){a.afGrid.renderer.NUMERIC={cell:function(b){return b;},comparator:function(c,b){c=Number(String(c).replace(/,/g,""));b=Number(String(b).replace(/,/g,""));return c-b;}};}(jQuery));(function(b){var a=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];b.afGrid.renderer.SYSTEM_DATE={cell:function(d){var c=d.split("-");
