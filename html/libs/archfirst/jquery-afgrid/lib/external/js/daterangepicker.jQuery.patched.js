...
$.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			{text: 'Any Date', dateStart: null, dateEnd: null },
	...
	rangeInput.bind("change.daterangepicker", options.onChange);

	rangeInput.bind("rangeInput-destroy", function(){
	    $(document).unbind("click.daterangeinput",documentClick);
        rp.unbind("click.daterangeinput");
    	rp.find('.range-start, range-end').datepicker('destroy');
	    rp.empty();
	    rp.parent().remove();
        rangeInput.unbind("change.daterangepicker").unbind("click.daterangepicker");
        rangeInput.unbind("rangeInput-destroy");
	});
	...
	$(document).bind("click.daterangeinput",documentClick);

	rp.bind("click.daterangeinput",function(){return false;}).hide();
}
...
function clickActions(rp, rpPickers, doneBtn, rangeInput, hideRP, options){
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			//custom date range
				...
				var dataDateStart = jQuery(this).data('dateStart');
				var dataDateEnd = jQuery(this).data('dateEnd');
				if (dataDateStart === null && dataDateEnd === null) {
    				rangeInput.val("");
    				hideRP();
    				return;
				}
				var dateStart = (typeof dataDateStart == 'string') ? Date.parse(jQuery(this).data('dateStart')) : jQuery(this).data('dateStart')();
				var dateEnd = (typeof dataDateEnd == 'string') ? Date.parse(jQuery(this).data('dateEnd')) : jQuery(this).data('dateEnd')();
