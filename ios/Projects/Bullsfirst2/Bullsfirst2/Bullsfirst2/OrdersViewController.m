...
{       
    NSString *errorString = @"Can't cancel this order!";
...
{    
    ...
    cell.date.text = @"26/10/00";
        cell.symbol.text = @"NEW";
        return cell;
    }
    else{
...
-(void) datePickerCleared:(DatePickerViewController *) controller
{
    ...
    {
        fromDate = nil;
        fromDateDropdownCTL.label.text = @"From: ";
    }
    else if (controller.tag == 2)
    {
        toDate = nil;
        toDateDropdownCTL.label.text = @"To: ";
    }
}
...
- (IBAction)hideFiletrsButtonPressed:(id)sender {
   // hideFiltersButton.imageView.image = [UIImage imageNamed:@"add_account_icon_normal.png"];
    
    UIButton *button = (UIButton*)sender;
    if(showFilterFlag)
    {
        [button setBackgroundImage:[UIImage imageNamed:@"more_info.png"] forState:UIControlStateNormal];
        showFilterFlag = FALSE;
        orderTBL.frame = CGRectMake(0, 200, 320, 186);
    }
    else{
        orderTBL.frame = CGRectMake(0, 254, 320, 186);
