...
*/
    ...
    s = [attrs objectForKey:(__bridge id)kSecAttrDescription];
    if (s != nil) {
        fprintf(stderr, "    description = '%s'\n", [s UTF8String]);
    }
    s = [attrs objectForKey:(__bridge id)kSecAttrComment];
    if (s != nil) {
        fprintf(stderr, "    comment     = '%s'\n", [s UTF8String]);
    }
    s = [attrs objectForKey:(__bridge id)kSecAttrLabel];
    if (s != nil) {
        fprintf(stderr, "    label       = '%s'\n", [s UTF8String]);
    }
    s = [attrs objectForKey:(__bridge id)kSecAttrAccount];
    if (s != nil) {
...
/*
    Internet:
        ...
        kSecAttrPath
*/
    s = [attrs objectForKey:(__bridge id)kSecAttrSecurityDomain];
    if (s != nil) {
        fprintf(stderr, "    domain      = '%s'\n", [s UTF8String]);
    }
    s = [attrs objectForKey:(__bridge id)kSecAttrServer];
    if (s != nil) {
        fprintf(stderr, "    server      = '%s'\n", [s UTF8String]);
    }
    s = [attrs objectForKey:(__bridge id)kSecAttrProtocol];
    if (s != nil) {
    ...
    n = [attrs objectForKey:(__bridge id)kSecAttrPort];
    if (n != nil) {
        fprintf(stderr, "    port        = %d\n", [n intValue]);
    }
    s = [attrs objectForKey:(__bridge id)kSecAttrPath];
    if (s != nil) {
...
/*
    generic:
        ...
        kSecAttrGeneric
*/
    s = [attrs objectForKey:(__bridge id)kSecAttrService];
    if (s != nil) {
        fprintf(stderr, "    service     = '%s'\n", [s UTF8String]);
    }
    d = [attrs objectForKey:(__bridge id)kSecAttrGeneric];
    if (d != nil) {
    ...
    // certificates that are /not/ associated with any identity.
{
    ...
    for (obj in identities) {
        ...
        identity = (__bridge SecIdentityRef) obj;
        ...
        [certificateDataToIdentityMap setObject:(__bridge id)identity forKey:(__bridge NSData *)identityCertificateData];
    ...
    for (obj in certificates) {
        ...
        certificate = (__bridge SecCertificateRef) obj;
        ...
        if ( [certificateDataToIdentityMap objectForKey:(__bridge NSData *)certificateData] == nil ) {
            [standaloneCertificates addObject:(__bridge id)certificate];
        }
    ...
    // of their certificates.
{
    ...
    leftIdentity  = (__bridge SecIdentityRef) left;
    rightIdentity = (__bridge SecIdentityRef) right;
    ...
    result = [(__bridge NSString *)leftSubject localizedCaseInsensitiveCompare:(__bridge NSString *)rightSubject];
    ...
    // A sort function that compares two certificates by their subject summaries.
{
    ...
    leftCertificate  = (__bridge SecCertificateRef) left;
    rightCertificate = (__bridge SecCertificateRef) right;
    ...
    result = [(__bridge NSString *)leftSubject localizedCaseInsensitiveCompare:(__bridge_transfer NSString *)rightSubject];
    ...
    // when it gets the KVO notification.
{
    ...
    if (err == noErr) {
        ...
        standaloneCertificates = [self _certificatesFromCertificates:(__bridge NSArray *)latestCertificates excludingIdentities:(__bridge NSArray *)latestIdentities];
        assert(standaloneCertificates != nil);
        
        if (notify) {
            [self willChangeValueForKey:@"identities"];
        }
        [self->_identities setArray:(__bridge NSArray *)latestIdentities];
